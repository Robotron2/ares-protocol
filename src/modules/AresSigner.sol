...
contract AresSigner {
    ...
    bytes32 public constant AUTH_TYPEHASH = keccak256("Authorize(address signer,bytes32 proposalHash,uint256 nonce)");
    ...
    function authorize(address signer, bytes32 proposalHash, bytes calldata signature) external returns (bool) {
        ...
        bytes32 structHash = keccak256(abi.encode(AUTH_TYPEHASH, signer, proposalHash, nonce));
